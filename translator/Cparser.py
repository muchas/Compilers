...
class Cparser(object):
    ...
    def p_program(selfelf, p):
        """program : program_blocks"""

        print (AST.Program(p[1]))

    def p_program_blocks(selfelf, p):
        """program_blocks : program_blocks program_block
                        | """
        if len(p) == 3:
            p[0] = AST.ProgramBlockList() if p[1] is None else p[1]
            p[0].append(p[2])
        else:
            p[0] = AST.ProgramBlockList()

    def p_program_block(selfelf, p):
        """program_block : declaration
                        | fundef
                        | instruction"""
